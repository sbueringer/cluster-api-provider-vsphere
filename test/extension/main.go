...
import (
	...
	"sigs.k8s.io/cluster-api-provider-vsphere/test/extension/handlers/lifecycle"
	"sigs.k8s.io/cluster-api-provider-vsphere/test/extension/handlers/topologymutation"
	"sigs.k8s.io/cluster-api-provider-vsphere/test/extension/third_party/cluster-api/exp/runtime/server"
)
...
var (
	...
	controllerName = "cluster-api-provider-vsphere-test-extension-manager"
...
func main() {
	...
	// Lifecycle hooks are not used for now and just shown here to give an
	// idea how they would be set up.
	// setupLifecycleHookHandlers(runtimeExtensionWebhookServer)
...
func setupLifecycleHookHandlers(runtimeExtensionWebhookServer *server.Server) { //nolint:unused // Intentionally kept for now.
	// Create the ExtensionHandlers for the lifecycle hooks
	// NOTE: it is not mandatory to group all the ExtensionHandlers using a struct, what is important
	// is to have HandlerFunc with the signature defined in sigs.k8s.io/cluster-api/exp/runtime/hooks/api/v1alpha1.
	lifecycleExtensionHandlers := lifecycle.NewExtensionHandlers()
