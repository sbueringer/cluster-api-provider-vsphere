...
RUNTIME_EXTENSION_VARIABLE_GEN_BIN := runtime-extension-variable-gen
RUNTIME_EXTENSION_VARIABLE_GEN := $(abspath $(TOOLS_BIN_DIR)/$(RUNTIME_EXTENSION_VARIABLE_GEN_BIN))
...
generate: ## Run all generate targets
	$(MAKE) generate-modules generate-manifests generate-go-deepcopy generate-variables generate-flavors
...
generate-variables: $(RUNTIME_EXTENSION_VARIABLE_GEN) ## Generate variables JSON
	$(MAKE) clean-generated-variables SRC_DIRS="./test/extension/api"
	$(RUNTIME_EXTENSION_VARIABLE_GEN) \
		--paths=./test/extension/api/... \
		--output-file=./test/extension/api/zz_generated.variables.json
...
.PHONY: clean-generated-variables
clean-generated-variables: ## Remove files generated by conversion-gen from the mentioned dirs. Example SRC_DIRS="./api/v1beta1"
	(IFS=','; for i in $(SRC_DIRS); do find $$i -type f -name 'zz_generated.variables.json' -exec rm -f {} \;; done)
...
.PHONY: $(RUNTIME_EXTENSION_VARIABLE_GEN_BIN)
$(RUNTIME_EXTENSION_VARIABLE_GEN_BIN): $(RUNTIME_EXTENSION_VARIABLE_GEN) ## Build a local copy of runtime-extension-variable-gen.
...
## We are forcing a rebuilt of runtime-extension-variable-gen via PHONY so that we're always using an up-to-date version.
.PHONY: $(RUNTIME_EXTENSION_VARIABLE_GEN)
$(RUNTIME_EXTENSION_VARIABLE_GEN): $(TOOLS_DIR)/go.mod # Build openapi-gen from tools folder.
	cd $(TOOLS_DIR); go build -tags=tools -o $(BIN_DIR)/$(RUNTIME_EXTENSION_VARIABLE_GEN_BIN) sigs.k8s.io/cluster-api-provider-vsphere/hack/tools/runtime-extension-variable-gen
